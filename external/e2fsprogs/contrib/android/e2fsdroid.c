...
int main(int argc, char *argv[])
{
	...
	if (android_configure) {
		...
		if(!strcmp(mountpoint, VENDOR_MNT) || !strcmp(mountpoint, ODM_MNT)) {
			...
			ino_add_xattr(fs, EXT2_ROOT_INO,  "security." XATTR_SELINUX_SUFFIX, VENDOR_SECLABLE, strlen(VENDOR_SECLABLE) + 1);
		} else if (!strcmp(mountpoint, SYSTEM_MNT) || !strcmp(mountpoint, PRODUCT_MNT) || !strcmp(mountpoint, VERSION_MNT) || !strcmp(mountpoint, CUST_MNT) || !strcmp(mountpoint, PRELOAD_MNT)){
			printf("Labeling %s as %s\n", mountpoint, SYSTEM_SECLABLE);
			ino_add_xattr(fs, EXT2_ROOT_INO,  "security." XATTR_SELINUX_SUFFIX, SYSTEM_SECLABLE, strlen(SYSTEM_SECLABLE) + 1);
		}
	}
#endif
